...
public void KVSaveOverview(char scorerName[MAX_NAME_LENGTH], char assisterName[MAX_NAME_LENGTH], char scoreid[32], char assistid[32], int scorerclient, int assisterclient)
{
	...
	if(!StrEqual(assisterName, "Owngoal"))
	{
		...
		char namesBuffer[1281];
		GetNames(scorerclient, namesBuffer, sizeof(namesBuffer));
		LeagueMatchKV.SetString("Other Names:", namesBuffer);
	}
	else
	{
		...
		char namesBuffer2[1281];
		GetNames(scorerclient, namesBuffer2, sizeof(namesBuffer2));
		LeagueMatchKV.SetString("Other Names:", namesBuffer2);
	}
	LeagueMatchKV.GoBack();
	...
	if(!StrEqual(assistid, "") && !StrEqual(assisterName, "Owngoal") && (GetClientTeam(scorerclient) == GetClientTeam(assisterclient)))
	{
		...
		char namesBuffer3[1281];
		GetNames(assisterclient, namesBuffer3, sizeof(namesBuffer3));
		LeagueMatchKV.SetString("Other Names:", namesBuffer3);
	}
	LeagueMatchKV.GoBack();
...
static stock void GetNames(int client, char[] output, int maxlen)
{
    output[0] = '\0';
    for(int i = 0; i <= iPlayerNames[client]; i++)
    {
        if(i == 1) Format(output, maxlen, "%s", PlayerNames[client][i]);
        else Format(output, maxlen, "%s | %s", output, PlayerNames[client][i]);
    }
}
